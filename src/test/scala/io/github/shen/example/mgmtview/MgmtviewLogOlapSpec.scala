...
class MgmtviewLogOlapSpec extends FlatSpec with GivenWhenThen with Matchers with Eventually with SparkStreamingSpec {
  ...
  val mgmtviewLogRecWithOrg = MgmtviewLogRecWithOrg(mgmtviewLogRec.get, orgInfoRec.get)

  "Valid Organization String" should "be parsed correctly" in {
    orgInfoRec.get shouldBe orgInfoRec2
  }

  "Invalid organization String" should "return empty" in {
    val orgStringLessField =  """230291138,哈尔滨先锋路分理处客户部,4,230291138,哈尔滨先锋路分理处客户部,
                                |230291138,哈尔滨先锋路分理处客户部,230291138,哈尔滨先锋路分理处客户部,
                                |230291138,哈尔滨先锋路分理处客户部,230100397,哈尔滨先锋路分理处,
                                |230000000,建行黑龙江省分行,111111111,中国建设银行,2015/9/26""".stripMargin.replaceAll("\n", "")
    val orgInfoRec = OrgInfoRec(orgStringLessField)
    orgInfoRec shouldBe empty
  }

  "'message' field in filebeat log" should "extracted properly"  in {
  ...
  "invalid json input doc" should "be skipped properly" in {
    val invalidJsonDoc = """"@timestamp":"2017-06-27T07:07:14.320Z",
    "beat":{"hostname":"W112PCO3UM12","name":"W112PCO3UM12", "version":"5.4.2"},
    "input_type":"log",
    "message":"2017-06-27 14:14:04,557|INFO |1498544044408|516067101|4600954220170627141138|AP121A001@TEST001002|AP121_test|230291138|1|||149|saiku|2|||",
    "offset":138,
    ...
    val lines = Seq(invalidJsonDoc)
    val msgLines = extractMessageFromFilebeatLog(sc.parallelize(lines)).collect()
    msgLines shouldBe empty
  }

  "Json input doc without 'message' field" should "be skipped properly" in {
    val jsonNoMessage = """{"@timestamp":"2017-06-27T07:07:14.320Z",
    "beat":{"hostname":"W112PCO3UM12","name":"W112PCO3UM12", "version":"5.4.2"},
    "input_type":"log",
    "msg":"2017-06-27 14:14:04,557|INFO |1498544044408|516067101|4600954220170627141138|AP121A001@TEST001002|AP121_test|230291138|1|||149|saiku|2|||",
    "offset":138,
    "source":"/home/ap/p12/shen/data/0.1og",
    "type":"log"
  }"""
    val lines = Seq(jsonNoMessage)
    val msgLines = extractMessageFromFilebeatLog(sc.parallelize(lines)).collect()
    msgLines shouldBe empty
  }

  "Valid MgmtviewLogRec string" should "be parsed properly" in {
    mgmtviewLogRec.get shouldBe mgmtviewLogRec2
  }

  "Invalid MgmtviewLogRec string" should "be skipped properly" in {
    val mgmtLogLessFields = "2017-06-27 14:14:04,557|INFO |1498544044408|516067101|4600954220170627141138|AP121A001@TEST001002|AP121_test|230291138|1|||149|saiku|2|"
    val rdd = sc.parallelize(Seq(mgmtLogLessFields))
    val arr = toMgmtviewLogRec(rdd).collect()
    arr shouldBe empty
  }
